...
async function main() {
    const commonPath = path.join(__dirname, '../src/data/i18n/common.json');
    let db = {};
    if (fs.existsSync(commonPath)) {
        db = JSON.parse(fs.readFileSync(commonPath, 'utf-8'));
    }

    const enStrings = db['en'];
    if (!enStrings) {
        throw new Error("No english strings found in common.json");
    }
    ...
    for (const locale of locales) {
        ...
        for (const [key, text] of Object.entries(enStrings)) {
            if (!db[locale][key]) {
                process.stdout.write(`Translating common [${locale}]: ${key}... `);
        ...
        fs.writeFileSync(commonPath, JSON.stringify(db, null, 2));
    }
    console.log("✅ Common translation complete!");
}
